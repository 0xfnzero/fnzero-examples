...
//! selected event, waits HOLD_SECONDS, then sells only the balance added by this run.
...
use sol_trade_sdk::{
    common::{clock::now_micros, GasFeeStrategy, SolanaRpcClient, TradeConfig},
    swqos::SwqosConfig,
    trading::{
...
async fn main() -> Result<()> {
    ...
    loop {
        if let Some(event) = queue.pop() {
            spin_count = 0;
            if let Some(event) = select_trade_event(
                event,
...
async fn execute_buy_hold_sell(
    ...
    let buy_params = SimpleBuyParams::new(
        ...
        DexParamEnum::PumpFun(build_buy_params_from_event(&event)),
        recent_blockhash,
    ...
    let sell_params = SimpleSellParams::new(
        ...
        DexParamEnum::PumpFun(sell_extension),
...
fn build_buy_params_from_event(event: &PumpFunTradeEvent) -> PumpFunParams {
    if event.is_created_buy {
        ...
        PumpFunParams::from_dev_trade_with_quote_mint(
            event.mint,
            event.token_amount,
            max_sol_cost,
            event.creator,
...
async fn build_fresh_sell_params(
    ...
    let params = PumpFunParams::from_mint_by_rpc(&client.infrastructure.rpc, &event.mint)
        .await
        .context("failed to refresh PumpFun state before sell; refusing stale event state")?;
